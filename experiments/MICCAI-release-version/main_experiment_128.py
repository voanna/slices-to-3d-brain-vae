...
parser.add_argument('--reconstruction-data-loss-weight', type=float, default=1.0,
                    help=' ')
parser.add_argument('--kl-latent-loss-weight', type=float, default=0.0,
                    help=' ')
...
class Encoder(nn.Module):
    def __init__(self, n_channels, gf_dim=16):
        ...
        self.res6 = ResBlockDown(gf_dim * 16, gf_dim * 32, act=False)
        self.res2_stdev = ResBlockDown(gf_dim * 16, gf_dim * 32, act=False)
...
mri_data_dir = '/scratch_net/biwinator02_third/voanna/curvature/data/MRI_slices_128_isotropic'
...
def make_train_loader(batch_size):
    ...
    train_data = datasets.ImageFolder(mri_data_dir + '/train/',
...
def make_test_loader(batch_size):
    ...
    test_data = datasets.ImageFolder(mri_data_dir + '/test/',
                                     transform=transforms.Compose([transforms.Grayscale(),
                                                                   transforms.ToTensor()]),
                                     # transforms.Normalize([0.5], [0.5])]),
                                     is_valid_file=subsample_volumes)
...
def main(args):
    ...
    encoder = Encoder(args.n_channels, gf_dim=8)
    decoder = Decoder(args.n_channels, gf_dim=8)
    ...
    train_loader = make_train_loader(batch_size=args.batch_size)
